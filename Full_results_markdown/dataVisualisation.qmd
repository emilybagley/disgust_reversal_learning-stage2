...
p_vals = pd.read_excel('pvalsForPlotting.xlsx')
p_vals = p_vals.rename(columns={"...1": "outcomeVar"})
FD_pvals=pd.read_csv("U:/Documents/Disgust learning project/github/disgust_reversal_learning-final/Full_results_markdown/videoRating_pvals_fearDisgust.csv")
expl_FD_pvals=pd.read_csv("U:/Documents/Disgust learning project/github/disgust_reversal_learning-final/Full_results_markdown/exploratory_videoRating_pvals_fearDisgust.csv")
...
points_pvals=pd.read_csv("U:/Documents/Disgust learning project/github/disgust_reversal_learning-final/Full_results_markdown/videoRating_pvals_points.csv")
...
def pVal_annot(outcomeVar):
    ...
    return fear_annot, points_annot
...
def expl_points_pVal_annot(Condition):
    fear_pVal=expl_points_pvals[expl_points_pvals.Condition==Condition]['trial_type[T.fear]'].iloc[0]
    if fear_pVal < 0.001:
        fear_annot='***'
    elif fear_pVal <0.01:
        fear_annot='**'
    ...
    disgust_pVal=expl_points_pvals[expl_points_pvals.Condition==Condition]['trial_type[T.disgust]'].iloc[0]
    if disgust_pVal < 0.001:
        disgust_annot='***'
    elif disgust_pVal <0.01:
        disgust_annot='**'
...
<h3>1. Checking sample demographics</h3>

<p>During data collection, exclusions (due to failed attention checks) were made on a rolling basis in order to best match the final sample with the target demographics (in terms of age, sex and psychiatric diagnosis). </p>
<p>We ran a series of checks to ensure that this was adequately done</p>
...
<h3>2. Video ratings task plots</h3>
...
<br>
<p>And in comparison to points ratings</p>
```{python}
#| code-fold: true
data=chosen_stim_df[['participant_no', 'trial_type', 'unpleasant_1', 'arousing_1', 'disgusting_1', 'frightening_1']].sort_values('trial_type')
...
<h3>3. Reversal learning task</h3>
...
<p><b>And finally, an exploratory test effected the effect of block-type on accuracy(percentage correct)</b></p>
```{python}
#| code-fold: true
palette = ["#F72585", "#3A0CA3", "#4CC9F0"]
dark_palette = ["#9B0F47", "#5E2E9D", "#2487B8"]
...
```{python}
#| echo: false
# add video ratings to dataframe to allow us to check for video ratings of outliers
...
task_summary=pd.merge(task_summary_short, stim_ratings_totals, on='participant_no', how='outer')

```


<p>Firstly, identify the outliers in the perseverative error outcome</p>
```{python}
#| code-fold: true
...
<p>And then plot where these outliers fall on other key task outcomes</p>
```{python}
#| code-fold: true
...
<p>And finally, assess whether they appear different on the video rating task</p>
