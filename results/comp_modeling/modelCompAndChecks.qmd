...
<p>Prior to this, data was processed, models were run and model-checks were extracted using the code within the modelFitting/ folder </p>
...
* models convergence outputs were extracted (effective sample size, rhat) (code in /modelDiagnostics folder - testingrhatneff.ipynb)
...
<p>BUT this also shows that no model produces no 'clear' winner (lower WAIC by >SEs)
<p>BUT 'stickiness' models consistently produce a better fit</p>
```{r}
...
<p>And looking within this, the single learning rate models are have better diagnostics (e.g., effective sample size is much higher for the single learning rate models))</p>
```{r}
#| label: neff
#| echo: true
#| code-fold: true
...
<p>Within this, the 'all parameter seperately' flavour of model is the most theoretically informative (as it allows us to compare the three feedback-types) and it has the lowest WAIC value, suggesting that despite having the most free parameters, it is the most parsimonious. </p>
