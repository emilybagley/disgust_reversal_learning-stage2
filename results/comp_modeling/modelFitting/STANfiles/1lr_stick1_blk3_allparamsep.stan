...
parameters {
  ...
  vector[N] d_alpha_pr;   // learning rate
  vector[N] d_beta_pr;   // inverse temperature
  vector[N] d_omega_pr; //stickiness

  vector[N] f_alpha_pr;   // learning rate
  vector[N] f_beta_pr;   // inverse temperature
  vector[N] f_omega_pr; //stickiness
...
transformed parameters {
  ...
  vector<lower=0, upper=1>[N] d_alpha;
  vector<lower=0, upper=10>[N] d_beta;
  vector<lower=-5, upper=5>[N] d_omega;

  vector<lower=0, upper=1>[N] f_alpha;
  vector<lower=0, upper=10>[N] f_beta;
  vector<lower=-5, upper=5>[N] f_omega;
  ...
  for (i in 1:N) {
    d_alpha[i]  = Phi_approx(mu_pr[1] + sigma[1] * d_alpha_pr[i]);
    d_beta[i]  = Phi_approx(mu_pr[2] + sigma[2] * d_beta_pr[i]) * 10;
    d_omega[i] = (Phi_approx(mu_pr[3] + sigma[3] * d_omega_pr[i])-0.5)*10; 

    f_alpha[i]  = Phi_approx(mu_pr[4] + sigma[4] * f_alpha_pr[i]);
    f_beta[i]  = Phi_approx(mu_pr[5] + sigma[5] * f_beta_pr[i]) * 10;
    f_omega[i] = (Phi_approx(mu_pr[6] + sigma[6] * f_omega_pr[i])-0.5)*10; 

    p_alpha[i]  = Phi_approx(mu_pr[7] + sigma[7] * p_alpha_pr[i]);
    p_beta[i]  = Phi_approx(mu_pr[8] + sigma[8] * p_beta_pr[i]) * 10;
    p_omega[i] = (Phi_approx(mu_pr[9] + sigma[9] * p_omega_pr[i])-0.5)*10; 
  }
}
...
model {
  ...
  d_alpha_pr ~ normal(0, 1);
  d_beta_pr ~ normal(0, 1);
  d_omega_pr ~ normal(0, 1);

  f_alpha_pr ~ normal(0, 1);
  f_beta_pr ~ normal(0, 1);
  f_omega_pr ~ normal(0, 1);
    ...
    for (bIdx in 1:Bsubj[i]) {  // new
      ...
      if (bIdx == 1) {
          for (t in 1:(Tsubj[i, bIdx])) {  // new
            //stickiness
            if (last_choice == 1) {
                  softMaxIntermed[1] = ev[1] + d_omega[i];
                  softMaxIntermed[2] = ev[2];}
            else {
                  softMaxIntermed[2] = ev[2] + d_omega[i];
                  softMaxIntermed[1] = ev[1];}

            // Softmax choice
            choice[i, bIdx, t] ~ categorical_logit(softMaxIntermed * d_beta[i]);
            ...
            ev[choice[i, bIdx, t]] += d_alpha[i] * PE;
          }
      }
      ...
      if (bIdx == 2) {
          for (t in 1:(Tsubj[i, bIdx])) {  // new
            //stickiness
            if (last_choice == 1) {
                  softMaxIntermed[1] = ev[1] + f_omega[i];
                  softMaxIntermed[2] = ev[2];}
            else {
                  softMaxIntermed[2] = ev[2] + f_omega[i];
                  softMaxIntermed[1] = ev[1];}

            // Softmax choice
            choice[i, bIdx, t] ~ categorical_logit(softMaxIntermed * f_beta[i]);
            last_choice = choice[i, bIdx, t];
            ...
            // Update expected value of chosen stimulus
            ev[choice[i, bIdx, t]] += f_alpha[i] * PE;
          }
      }
...
generated quantities {
  ...
  real<lower=0, upper=1>  mu_d_alpha;
  real<lower=0, upper=10> mu_d_beta;
  real<lower=-5, upper=5> mu_d_omega;

  real<lower=0, upper=1>  mu_f_alpha;
  real<lower=0, upper=10> mu_f_beta;
  real<lower=-5, upper=5> mu_f_omega;
  ...
  mu_d_alpha = Phi_approx(mu_pr[1]);
  mu_d_beta = Phi_approx(mu_pr[2]) * 10;
  mu_d_omega = (Phi_approx(mu_pr[3])-0.5)*10;

  mu_f_alpha = Phi_approx(mu_pr[4]);
  mu_f_beta = Phi_approx(mu_pr[5]) * 10;
  mu_f_omega = (Phi_approx(mu_pr[6])-0.5)*10;

  mu_p_alpha = Phi_approx(mu_pr[7]);
  mu_p_beta = Phi_approx(mu_pr[8]) * 10;
  mu_p_omega = (Phi_approx(mu_pr[9])-0.5)*10;
      ...
      for (bIdx in 1:Bsubj[i]) {
        ...
        if (bIdx == 1) {
            for (t in 1:(Tsubj[i, bIdx])) {
              ...
              // Softmax choice
              log_lik[i]  += categorical_logit_lpmf(choice[i, bIdx, t] | softMaxIntermed * d_beta[i]);

              // generate posterior prediction for current trial
              y_pred[i, bIdx, t] = categorical_rng(softmax(softMaxIntermed * d_beta[i]));
        ...
        if (bIdx == 2) {
            for (t in 1:(Tsubj[i, bIdx])) {
              ...
              // Softmax choice
              log_lik[i]  += categorical_logit_lpmf(choice[i, bIdx, t] | softMaxIntermed * f_beta[i]);

              // generate posterior prediction for current trial
              y_pred[i, bIdx, t] = categorical_rng(softmax(softMaxIntermed * f_beta[i]));
