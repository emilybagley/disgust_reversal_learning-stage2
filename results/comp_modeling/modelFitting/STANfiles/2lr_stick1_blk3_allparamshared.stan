...
parameters {
  ...
  vector[N] alpha_rew_pr;   // pos learning rate
  vector[N] alpha_pun_pr;   // neg learning rate
...
transformed parameters {
  ...
  for (i in 1:N) {
    alpha_rew[i]  = Phi_approx(mu_pr[1] + sigma[1] * alpha_rew_pr[i]);
    alpha_pun[i]  = Phi_approx(mu_pr[2] + sigma[2] * alpha_pun_pr[i]);
    beta[i]  = Phi_approx(mu_pr[3] + sigma[3] * beta_pr[i]) * 10;
    omega[i] = (Phi_approx(mu_pr[4] + sigma[4] * omega_pr[i])-0.5)*10;
  }
}
    ...
    if (bIdx == 1) {
      for (t in 1:Tsubj[i, bIdx]) {
        ...
        if (outcome[i, bIdx, t] > 0)
          ev[choice[i, bIdx, t]] += alpha_rew[i] * PE;
        else
          ev[choice[i, bIdx, t]] += alpha_pun[i] * PE;
      }
    }
...
// Initialize all the variables to avoid NULL values
  ...
  mu_alpha_rew = Phi_approx(mu_pr[1]);
  mu_alpha_pun = Phi_approx(mu_pr[2]);
  mu_beta = Phi_approx(mu_pr[3]) * 10;
  mu_omega = (Phi_approx(mu_pr[4])-0.5)*10;
