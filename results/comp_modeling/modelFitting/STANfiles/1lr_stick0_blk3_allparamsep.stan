...
transformed parameters {
  ...
  for (i in 1:N) {
    ...
    f_alpha[i]  = Phi_approx(mu_pr[3] + sigma[3] * f_alpha_pr[i]);
    f_beta[i]  = Phi_approx(mu_pr[4] + sigma[4] * f_beta_pr[i]) * 10;
    p_alpha[i]  = Phi_approx(mu_pr[5] + sigma[5] * p_alpha_pr[i]);
...
generated quantities {
  ...
  mu_f_alpha = Phi_approx(mu_pr[3]);
  mu_f_beta = Phi_approx(mu_pr[4]) * 10;
  mu_p_alpha = Phi_approx(mu_pr[5]);
