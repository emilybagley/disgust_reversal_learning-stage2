...
transformed parameters {
  ...
  for (i in 1:N) {
    ...
    p_alpha[i]  = Phi_approx(mu_pr[3] + sigma[3] * p_alpha_pr[i]);
    p_beta[i]  = Phi_approx(mu_pr[4] + sigma[4] * p_beta_pr[i]) * 10;
  }
}
...
generated quantities {
  ...
  mu_p_alpha = Phi_approx(mu_pr[3]);
  mu_p_beta = Phi_approx(mu_pr[4]) * 10;
