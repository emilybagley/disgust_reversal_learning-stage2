...
transformed parameters {
  ...
  for (i in 1:N) {
    ...
    p_alpha_rew[i]  = Phi_approx(mu_pr[4] + sigma[4] * p_alpha_rew_pr[i]);
    p_alpha_pun[i]  = Phi_approx(mu_pr[5] + sigma[5] * p_alpha_pun_pr[i]);
    p_beta[i]  = Phi_approx(mu_pr[6] + sigma[6] * p_beta_pr[i]) * 10;
  }
}
  ...
  for (i in 1:N) {
    ...
    if (bIdx == 1) {
      for (t in 1:Tsubj[i, bIdx]) {
        // Softmax choice
        choice[i, bIdx, t] ~ categorical_logit(ev * e_beta[i]);
...
// Initialize all the variables to avoid NULL values
  ...
  mu_p_alpha_rew = Phi_approx(mu_pr[4]);
  mu_p_alpha_pun = Phi_approx(mu_pr[5]);
  mu_p_beta = Phi_approx(mu_pr[6]) * 10;
    ...
    for (i in 1:N) {
      ...
      for (bIdx in 1:Bsubj[i]) {
          ...
          if(bIdx == 1){
            for (t in 1:Tsubj[i, bIdx]) {
                // Softmax choice
                log_lik[i]  += categorical_logit_lpmf(choice[i, bIdx, t] | ev * e_beta[i]);

                // generate posterior prediction for current trial
                y_pred[i, bIdx, t] = categorical_rng(softmax(ev * e_beta[i]));
