...
parameters {
  ...
  vector[N] e_alpha_rew_pr;   // pos learning rate
  vector[N] e_alpha_pun_pr;   // neg learning rate
...
transformed parameters {
  ...
  vector<lower=0, upper=1>[N] e_alpha_rew;
  vector<lower=0, upper=1>[N] e_alpha_pun;
  ...
  for (i in 1:N) {
    e_alpha_rew[i]  = Phi_approx(mu_pr[1] + sigma[1] * e_alpha_rew_pr[i]);
    e_alpha_pun[i]  = Phi_approx(mu_pr[2] + sigma[2] * e_alpha_pun_pr[i]);
    e_beta[i]  = Phi_approx(mu_pr[3] + sigma[3] * e_beta_pr[i]) * 10;
    e_omega[i] = (Phi_approx(mu_pr[4] + sigma[4] * e_omega_pr[i])-0.5)*10;
    
    p_alpha_rew[i]  = Phi_approx(mu_pr[5] + sigma[5] * p_alpha_rew_pr[i]);
    p_alpha_pun[i]  = Phi_approx(mu_pr[6] + sigma[6] * p_alpha_pun_pr[i]);
    p_beta[i]  = Phi_approx(mu_pr[7] + sigma[7] * p_beta_pr[i]) * 10;
    p_omega[i] = (Phi_approx(mu_pr[8] + sigma[8] * p_omega_pr[i])-0.5)*10;
  }
}
...
model {
  ...
  e_alpha_rew_pr ~ normal(0, 1);
  e_alpha_pun_pr ~ normal(0, 1);
    ...
    if (bIdx == 1) {
      for (t in 1:Tsubj[i, bIdx]) {
        ...
        if (outcome[i, bIdx, t] > 0)
          ev[choice[i, bIdx, t]] += e_alpha_rew[i] * PE;
        else
          ev[choice[i, bIdx, t]] += e_alpha_pun[i] * PE;
      }
    }
...
generated quantities {
  ...
  real<lower=0, upper=1>  mu_e_alpha_rew;
  real<lower=0, upper=1>  mu_e_alpha_pun;
...
// Initialize all the variables to avoid NULL values
  ...
  mu_e_alpha_rew = Phi_approx(mu_pr[1]);
  mu_e_alpha_pun = Phi_approx(mu_pr[2]);
  mu_e_beta = Phi_approx(mu_pr[3]) * 10;
  mu_e_omega = (Phi_approx(mu_pr[4])-0.5)*10;

  mu_p_alpha_rew = Phi_approx(mu_pr[5]);
  mu_p_alpha_pun = Phi_approx(mu_pr[6]);
  mu_p_beta = Phi_approx(mu_pr[7]) * 10;
  mu_p_omega = (Phi_approx(mu_pr[8])-0.5)*10;
