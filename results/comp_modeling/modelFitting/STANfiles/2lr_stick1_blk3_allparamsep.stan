...
transformed parameters {
  ...
  for (i in 1:N) {
    ...
    d_omega[i] = (Phi_approx(mu_pr[4] + sigma[4] * d_omega_pr[i])-0.5)*10;
    
    f_alpha_rew[i]  = Phi_approx(mu_pr[5] + sigma[5] * f_alpha_rew_pr[i]);
    f_alpha_pun[i]  = Phi_approx(mu_pr[6] + sigma[6] * f_alpha_pun_pr[i]);
    f_beta[i]  = Phi_approx(mu_pr[7] + sigma[7] * f_beta_pr[i]) * 10;
    f_omega[i] = (Phi_approx(mu_pr[8] + sigma[8] * f_omega_pr[i])-0.5)*10;
    
    p_alpha_rew[i]  = Phi_approx(mu_pr[9] + sigma[9] * p_alpha_rew_pr[i]);
    p_alpha_pun[i]  = Phi_approx(mu_pr[10] + sigma[10] * p_alpha_pun_pr[i]);
    p_beta[i]  = Phi_approx(mu_pr[11] + sigma[11] * p_beta_pr[i]) * 10;
    p_omega[i] = (Phi_approx(mu_pr[12] + sigma[12] * p_omega_pr[i])-0.5)*10;
  }
}
...
// Initialize all the variables to avoid NULL values
  ...
  mu_d_omega = (Phi_approx(mu_pr[4])-0.5)*10;

  mu_f_alpha_rew = Phi_approx(mu_pr[5]);
  mu_f_alpha_pun = Phi_approx(mu_pr[6]);
  mu_f_beta = Phi_approx(mu_pr[7]) * 10;
  mu_f_omega = (Phi_approx(mu_pr[8])-0.5)*10;

  mu_p_alpha_rew = Phi_approx(mu_pr[9]);
  mu_p_alpha_pun = Phi_approx(mu_pr[10]);
  mu_p_beta = Phi_approx(mu_pr[11]) * 10;
  mu_p_omega = (Phi_approx(mu_pr[12])-0.5)*10;
