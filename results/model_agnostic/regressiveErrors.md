...
Assess and correct for skewness in regressive error outcome
</h3>
...
![](regressiveErrors_files/figure-commonmark/Skewness-1.jpeg)
...
```
    ...
    0   565.219279  597.273718                  604.201273   

       feedback_randint_randslope  feedback_fractals_randint  
    0                  604.201273                 839.056473  
...
fractals_randint_randslope <- glmer(pos_regressive_er ~ block_type + (block_type|participant_no) + (1|fractals), data=task_summary, family=Gamma(link="inverse"))
...
model_names <- c("basic model", "feedback_randint", "fractals_randint", "feedback_randint_randslope", "fractals_randint_randslope", "feedback_fractals_randint_randslope")
...
```
    ...
    5          fractals_randint_randslope 4509.141
    6 feedback_fractals_randint_randslope 4515.996
...
```
    ...
    [1] "Winning models: Gamma (inverse) feedback_randint_randslope no_covariate"
...
```
    ...
    .sig07                    NA         NA
    .sigma                    NA         NA
    (Intercept)       0.44378516 0.54422188
    block_typeFear   -0.05681566 0.05387291
    block_typePoints -0.05167579 0.05677225
...
```
    ...
    Points vs Fear: T = -1.2528531526426387, CI95% = [-1.5   0.33], p = 0.2111226627524318, dof = 339
...
```
    ...
    Points vs Fear: BF01(339) = 7.575757575757575
...
```
       ...
       basic_model   randslope  fractals_randint_randslope  \
    0   585.869188  617.884096                   624.81165   

       feedback_randint_randslope  feedback_fractals_randint  
    0                   624.81165                 859.565855  
...
```
    ...
    LM Statistic 21.012327189237865
    LM-Test p-value 0.22573937517351214
    F-Statistic 1.2397463138198446
    F-Test p-value 0.22572279544930554
...
```
                           ...
                           Model      BIC
    1            Gamma (inverse) 4596.202
    3 inverse gaussian (inverse) 5905.345
    2     inverse gaussian (log) 5905.383
...
#fractals_randint_randslope <- glmer(pos_regressive_er ~ block_type + valence_diff + arousal_diff + valence_habdiff + (block_type|participant_no) + (1|fractals), data=task_summary, family=Gamma(link="inverse"))
...
```
                 ...
                 Model      BIC
    2 fractals_randint 4588.165
    1      basic model 4596.202
...
model_names <- c(
  ...
  "digit_span_age_covariate",
  "sex_digit_span_age_covariate"
)
...
```
                             ...
                             Model      BIC
    1                 no_covariate 4588.165
    3                sex_covariate 4594.168
    4         digit_span_covariate 4594.616
    2                age_covariate 4594.707
    6     sex_digit_span_covariate 4600.695
    5            sex_age_covariate 4600.713
    7     digit_span_age_covariate 4601.235
    8 sex_digit_span_age_covariate 4607.356
...
```
    ...
    [1] "Winning models: Gamma (inverse) fractals_randint no_covariate"
...
```
    ...
    .sig02                     NA         NA
    .sigma                     NA         NA
    (Intercept)       0.389770855 0.47414824
    block_typeFear   -0.018657661 0.01019103
    block_typePoints -0.002857374 0.02632035
    valence_diff     -0.017179874 0.01973394
    arousal_diff     -0.029391861 0.01976368
    valence_habdiff  -0.013465145 0.01446283
...
``` python
print(pearsonr(task_summary.mean_regressive_er, task_summary.percentage_correct))
```
...
```
       ...
       no_covariate  digit_span_covariate  age_covariate  sex_covariate  \
    0    574.788389            580.773793     580.905279      580.97193   

       digit_span_age_covariate  sex_age_covariate  sex_digit_span_covariate  \
    0                587.116863         587.119632                587.134876   
...
```
                           ...
                           Model      BIC
    1            Gamma (inverse) 4453.507
    3 inverse gaussian (inverse) 5767.006
    2     inverse gaussian (log) 5767.006
...
#fractals_randint_randslope <- glmer(pos_regressive_er ~ block_type + (block_type|participant_no) + (1|fractals), data=explore_df, family=Gamma(link="inverse"))
...
```
                 ...
                 Model      BIC
    1      basic model 4453.507
    2 feedback_randint 4460.426
...
```
                     ...
                     Model      BIC
    1         no_covariate 4453.507
    2 digit_span_covariate 4459.840
...
```
    ...
    Points vs Fear: T = -1.1605397299106521, CI95% = [-1.08  0.28], p = 0.24665795728076312, dof=334
...
```
    ...
    Points vs Fear: BF01 (334) = 8.403361344537815
...
![](regressiveErrors_files/figure-commonmark/Skewness%20sensitivity-1.jpeg)
...
This time the assumptions are not violated - so a generalized model does
not need to be run.
</h4>
...
```
    ...
    Points vs Fear: T = -0.6018387563633923, CI95% = [-0.3   0.16], p = 0.5477702170939132, dof = 279
