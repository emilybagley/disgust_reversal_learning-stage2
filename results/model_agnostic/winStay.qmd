...
<p>And because the result is null, we also calculate a Bayes factor: </p>
```{python}
...
<p>The originally planned outlier criteria is not fit for purpose due to the large skew of the win-stay outcome. Instead, we run a sensitivity analysis excluding data-points that are outliers in terms of accuracy</p>

<p>Exclude outliers according to this definition</p>
```{python}
#| code-fold: true
...
<p>Results of this winning model</p>
```{python}
print(results.summary())
```

<p>And once age is excluded</p>
```{python}
print(no_covariate.summary())
```

<p>With Bayesian t-tests</p>
<p>Firstly for disgust vs fear:</p>

```{python}
#| label: disgust vs fear bayes factor alt outlier
#| echo: true
#| code-fold: true
...
```{python}
#| label: disgust vs points bayes factor alt outlier
#| echo: true
#| code-fold: true
...
<p>We also look at fear vs points (which is not directly assessed by the model)</p>
```{python}
#| label: points vs fear ttest alt outlier
#| echo: true
#| code-fold: true
...
```{python}
#| label: points vs fear bayes factor alt outlier
#| echo: true
#| code-fold: true
print(f"Points vs Fear: BF01({ttest['dof'].iloc[0]}) = {bf_null}")
```
...
<p>We also test whether win-stay probability correlates with this accuracy measure (which it does). </p>
```{python}
#| code-fold: true
...
<h3> We also include the original outlier-free analysis for completeness </h3>
...
<p>Assess and correct for skewness in win-stay outcome (excluding outliers)</p>
