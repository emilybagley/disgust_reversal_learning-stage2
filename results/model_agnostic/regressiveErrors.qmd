...
<p>And also show that, as expected, accuracy is negatively correlated with the regressive error outcome.</p>
```{python}
#| code-fold: true
...
<p>Correct skew of this outlier-free dataset </p>
```{python}
...
<p>Extract confidence intervals</p>
```{r}
#| label: confidence intervals - alt outlier
#| echo: true
print(confint.merMod(no_covariate, method='Wald'))
```
...
<br>
<h3>Assess and correct for skewness in regressive error outcome (excluding outliers)</h3>
```{python}
...
<h4>This time the assumptions are not violated - so a generalized model does not need to be run. </h4>
<p>Shapiro-Wilk test of normality of residuals</p>
```{python}
